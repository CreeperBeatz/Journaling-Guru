...
6. **Voice (4–5 days)** — voice_sessions, `/voice/session` ephemeral-token endpoint, WebRTC + data channel on frontend, transcript capture + finalize, mapping LLM call, per-user minute quota.
...
- **Voice**: confirm OpenAI key never appears in browser DevTools network tab. Disconnect mid-call → `voice_sessions.status='failed'`. Complete call → entries created and mapped to correct questions.
